                       ...
                       AVFormatParameters *ap)
{
    ...
    jv->state = JVDemuxContext::JV_AUDIO;
    return 0;
}
...
static int read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    while (!url_feof(s->pb) && jv->pts < ast->nb_index_entries) {
        ...
        switch(jv->state) {
        case JVDemuxContext::JV_AUDIO:
            jv->state = JVDemuxContext::JV_VIDEO;//++;
            if (jvf->audio_size ) {
        ...
        case JVDemuxContext::JV_VIDEO:
            jv->state = JVDemuxContext::JV_PADDING;//++;
        ...
        case JVDemuxContext::JV_PADDING:
