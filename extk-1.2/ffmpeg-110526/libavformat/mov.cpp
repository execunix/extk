...
static int mov_read_chpl(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    for (i = 0; i < nb_chapters; i++) {
        ...
        str[str_len] = 0;
        ff_new_chapter(c->fc, i, AVRationalArg(1,10000000), start, AV_NOPTS_VALUE, str);
    }
    return 0;
}
...
static int mov_read_dac3(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    st->codec->channels = channels_tbl[acmod] + lfeon;
...
int ff_mov_read_stsd_entries(MOVContext *c, AVIOContext *pb, int entries)
{
    ...
    for(pseudo_stream_id=0; pseudo_stream_id<entries; pseudo_stream_id++) {
        ...
        if(st->codec->codec_type==AVMEDIA_TYPE_VIDEO) {
            ...
            MOVAtom fake_atom = { 0, /*.size = */size - (avio_tell(pb) - start_pos) };
...
struct MOVAtomArg : public MOVAtom{
	MOVAtomArg(uint32_t t, int64_t s) { type = t; size = s; }
};
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
 ...
 retry:
    ...
    if (!sample) {
        mov->found_mdat = 0;
        if (s->pb->seekable||
            mov_read_default(mov, s->pb, MOVAtomArg(AV_RL32("root"), INT64_MAX)) < 0 ||
            url_feof(s->pb))
            return AVERROR_EOF;
