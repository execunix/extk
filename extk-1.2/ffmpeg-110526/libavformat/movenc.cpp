...
static int mov_write_stts_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    if (track->enc->codec_type == AVMEDIA_TYPE_AUDIO && !track->audio_vbr) {
        ...
        stts_entries = (MOVStts*)av_malloc(track->entry * sizeof(*stts_entries)); /* worst case */
        for (i=0; i<track->entry; i++) {
...
static int mov_write_hdlr_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    avio_wb32(pb, 0); /* Version & flags */
    avio_write(pb, (const unsigned char*)hdlr, 4); /* handler */
    ...
    avio_write(pb, (const unsigned char*)descr, strlen(descr)); /* handler description */
    if (track && track->mode != MODE_MOV)
...
static int mov_write_udta_sdp(AVIOContext *pb, AVFormatContext *ctx, int index)
{
    ...
    avio_wb32(pb, len + 8);
    ffio_wfourcc(pb, "sdp ");
    avio_write(pb, (unsigned char*)buf, len);
    return len + 24;
}
                                  ...
                                  const char *tag, const char *str)
{
    ...
    if (!strcmp(tag, "yrrc"))
        ...
        avio_write(pb, (unsigned char*)t->value, strlen(t->value)+1); /* UTF8 string value */
        if (!strcmp(tag, "albm") &&
...
static int mov_write_chpl_tag(AVIOContext *pb, AVFormatContext *s)
{
    ...
    for (i = 0; i < nb_chapters; i++) {
        ...
        AVMetadataTag *t;
        avio_wb64(pb, av_rescale_q(c->start, c->time_base, AVRationalArg(1,10000000)));

        if ((t = av_metadata_get(c->metadata, "title", NULL, 0))) {
            int len = FFMIN(strlen(t->value), 255);
            avio_w8(pb, len);
            avio_write(pb, (unsigned char*)t->value, len);
        } else
            avio_w8(pb, 0);
    }
    return updateSize(pb, pos);
...
int ff_mov_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (!(trk->entry % MOV_INDEX_CLUSTER_SIZE)) {
        trk->cluster = (MOVIentry*)av_realloc(trk->cluster, (trk->entry + MOV_INDEX_CLUSTER_SIZE) * sizeof(*trk->cluster));
        if (!trk->cluster)
            return -1;
    }
...
static void mov_create_chapter_track(AVFormatContext *s, int tracknum)
{
    ...
    for (i = 0; i < s->nb_chapters; i++) {
        ...
        int64_t end = av_rescale_q(c->end, c->time_base, AVRationalArg(1,MOV_TIMESCALE));
        pkt.pts = pkt.dts = av_rescale_q(c->start, c->time_base, AVRationalArg(1,MOV_TIMESCALE));
...
static const AVCodecTag* const codec_tag_mov[] = {codec_movvideo_tags, codec_movaudio_tags, 0};
#if CONFIG_MOV_MUXER
AVOutputFormat ff_mov_muxer = {
	...
	NULL, // dummy
	NULL, // interleave_packet
    codec_tag_mov, // {codec_movvideo_tags, codec_movaudio_tags, 0},
};
#endif

static const AVCodecTag* const codec_tag_3gp[] = {codec_3gp_tags, 0};
#if CONFIG_TGP_MUXER
AVOutputFormat ff_tgp_muxer = {
	...
	NULL, // dummy
	NULL, // interleave_packet
    codec_tag_3gp, // {codec_3gp_tags, 0},
};
#endif

static const AVCodecTag* const codec_tag_mp4[] = {ff_mp4_obj_type, 0};
#if CONFIG_MP4_MUXER
AVOutputFormat ff_mp4_muxer = {
	...
	NULL, // dummy
	NULL, // interleave_packet
    codec_tag_mp4, // {ff_mp4_obj_type, 0},
};
#endif
...
static const AVCodecTag* const codec_tag_ipod[] = {codec_ipod_tags, 0};
...
AVOutputFormat ff_ipod_muxer = {
	...
	NULL, // dummy
	NULL, // interleave_packet
    codec_tag_ipod, // {codec_ipod_tags, 0},
};
#endif
