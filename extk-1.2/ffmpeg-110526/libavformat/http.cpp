...
static int http_open_cnx(URLContext *h)
{
    ...
    proxy_path = "http_proxy"/*getenv("http_proxy")*/;
    use_proxy = (proxy_path != NULL) && !"no_proxy"/*getenv("no_proxy")*/ &&
                        ...
                        int *new_location)
{
    ...
    if (line_count == 0) {
        ...
        *p = '\0';
        ...
        while (isspace(*p))
            p++;
        if (!_stricmp(tag, "Location")) {
            strcpy(s->location, p);
            *new_location = 1;
        } else if (!_stricmp (tag, "Content-Length") && s->filesize == -1) {
            s->filesize = atoll(p);
        } else if (!_stricmp (tag, "Content-Range")) {
        ...
        } else if (!_stricmp (tag, "Transfer-Encoding") && !_strnicmp(p, "chunked", 7)) {
            s->filesize = -1;
            s->chunksize = 0;
        } else if (!_stricmp (tag, "WWW-Authenticate")) {
            ff_http_auth_handle_header(&s->auth_state, tag, p);
        } else if (!_stricmp (tag, "Authentication-Info")) {
            ff_http_auth_handle_header(&s->auth_state, tag, p);
        } else if (!_stricmp (tag, "Connection")) {
            if (!strcmp(p, "close"))
                s->willclose = 1;
        }
    }
