...
static av_cold int concat_close(URLContext *h)
{
    int err = 0;
    size_t i;
    struct concat_data  *data  = (concat_data*)h->priv_data;
...
static av_cold int concat_open(URLContext *h, const char *uri, int flags)
{
    ...
    /* creating data */
    if (!(data = (concat_data*)av_mallocz(sizeof(*data))))
    ...
    if (!(nodes = (concat_nodes*)av_malloc(sizeof(*nodes) * len))) {
        av_freep(&h->priv_data);
        return AVERROR(ENOMEM);
    } else
        data->nodes = nodes;
    ...
    if (err < 0)
        concat_close(h);
    else if (!(nodes = (concat_nodes*)av_realloc(nodes, data->length * sizeof(*nodes)))) {
        concat_close(h);
        err = AVERROR(ENOMEM);
    } else
        data->nodes = nodes;
