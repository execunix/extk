...
#define transpose(x) ((x>>3) | ((x&7)<<3))
    ...
    v->is_intra_base = (uint8_t*)av_malloc(sizeof(v->is_intra_base[0]) * 2 * s->mb_stride);
    ...
    if (avctx->codec_id == CODEC_ID_VC1) {
        ...
        if(IS_MARKER(AV_RB32(buf))){ /* frame starts with marker and needs to be parsed */
            ...
            for(start = buf, end = buf + buf_size; next < end; start = next){
                ...
                case VC1_CODE_SLICE: {
                    int buf_size3;
                    slices = (slices_t*)av_realloc(slices, sizeof(*slices) * (n_slices+1));
                    if (!slices) goto err;
...
AVCodec ff_vc1_decoder = {
    ...
    ff_hwaccel_pixfmt_list_420, // pix_fmts
