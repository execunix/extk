...
static av_cold int build_modpred(Indeo3DecodeContext *s)
{
    ...
    if (!(s->ModPred = (uint8_t*)av_malloc(8 * 128)))
    ...
    if (!(s->corrector_type = (uint8_t*)av_malloc(24 * 256)))
        return AVERROR(ENOMEM);
...
static av_cold int iv_alloc_frames(Indeo3DecodeContext *s)
{
    ...
    av_freep(&s->buf);
    if(!(s->buf = (uint8_t*)av_malloc(bufsize)))
        return AVERROR(ENOMEM);
...
{
    Indeo3DecodeContext *s = (Indeo3DecodeContext*)avctx->priv_data;
    int ret = 0;
