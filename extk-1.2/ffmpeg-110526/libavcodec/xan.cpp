...
static av_cold int xan_decode_init(AVCodecContext *avctx)
{
    ...
    s->buffer1 = (uint8_t*)av_malloc(s->buffer1_size);
    if (!s->buffer1)
        return AVERROR(ENOMEM);
    s->buffer2_size = avctx->width * avctx->height;
    s->buffer2 = (uint8_t*)av_malloc(s->buffer2_size + 130);
    if (!s->buffer2) {
        av_freep(&s->buffer1);
        return AVERROR(ENOMEM);
    }
        ...
        while (buf_end - buf > 8 && tag != VGA__TAG) {
            ...
            case PALT_TAG:
                ...
                if (s->palettes_count >= PALETTES_MAX)
                    return AVERROR_INVALIDDATA;
                tmpptr = (unsigned*)av_realloc(s->palettes, (s->palettes_count + 1) * AVPALETTE_SIZE);
                if (!tmpptr)
                    return AVERROR(ENOMEM);
                s->palettes = tmpptr;
