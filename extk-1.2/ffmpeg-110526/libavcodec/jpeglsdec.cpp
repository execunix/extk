...
static inline int ls_get_code_regular(GetBitContext *gb, JLSState *state, int Q){
    ...
    if(!state->onenear && !k && (2 * state->B[Q] <= -state->N[Q]))
        ret = -(ret + 1);
...
static inline void ls_decode_line(JLSState *state, MJpegDecodeContext *s, void *last, void *dst, int last2, int w, int stride, int comp, int bits){
    ...
    while(x < w) {
        ...
        if(state->onenear){
            if(pred < -state->onenear)
                pred += state->range * state->twonear;
            else if(pred > state->maxval + state->onenear)
                pred -= state->range * state->twonear;
...
int ff_jpegls_decode_picture(MJpegDecodeContext *s, int onenear, int point_transform, int ilv){
    int i, t = 0;
...
//    av_log(s->avctx, AV_LOG_DEBUG, "JPEG-LS params: %ix%i NEAR=%i MV=%i T(%i,%i,%i) RESET=%i, LIMIT=%i, qbpp=%i, RANGE=%i\n",s->width,s->height,state->onenear,state->maxval,state->T1,state->T2,state->T3,state->reset,state->limit,state->qbpp, state->range);
