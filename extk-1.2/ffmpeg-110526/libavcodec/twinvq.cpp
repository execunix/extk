                         ...
                         int ch, float *out, float gain, enum FrameType ftype)
{
    ...
    const float ftype_tbl[] = {0.4, 0.35, 0.28};
    float val = /*((const float []) {0.4, 0.35, 0.28})*/ftype_tbl[ftype];
...
static av_cold void init_mdct_win(TwinContext *tctx)
{
    ...
    tctx->tmp_buf  = (float*)av_malloc(mtab->size            * sizeof(*tctx->tmp_buf));

    tctx->spectrum  = (float*)av_malloc(2*mtab->size*channels*sizeof(float));
    tctx->curr_frame = (float*)av_malloc(2*mtab->size*channels*sizeof(float));
    tctx->prev_frame  = (float*)av_malloc(2*mtab->size*channels*sizeof(float));

    for (i = 0; i < 3; i++) {
        int m = 4*mtab->size/mtab->fmode[i].sub;
        double freq = 2*M_PI/m;
        tctx->cos_tabs[i] = (float*)av_malloc((m/4)*sizeof(*tctx->cos_tabs));
...
static av_cold void init_bitstream_params(TwinContext *tctx)
{
    ...
    for (frametype = FT_SHORT; frametype <= FT_PPC; (*(int*)frametype)++)
