...
{
    PadContext *pad = (PadContext*)ctx->priv;
...
static int config_input(AVFilterLink *inlink)
{
    ...
    ff_fill_line_with_color(pad->line, pad->line_step, pad->w, pad->color,
                            (PixelFormat)inlink->format, rgba_color, &is_packed_rgba, NULL);
...
{
    PadContext *pad = (PadContext*)outlink->src->priv;
...
{
    PadContext *pad = (PadContext*)inlink->dst->priv;
...
{
    PadContext *pad = (PadContext*)link->dst->priv;
    int bar_y, bar_h = 0;
