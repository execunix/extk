...
{
    BufferSourceContext *c = (BufferSourceContext*)buffer_filter->priv;
    ...
    if (picref->video->w != c->w || picref->video->h != c->h || picref->format != c->pix_fmt) {
        ...
        scale->inputs[0]->format = picref->format;
        c->pix_fmt = (PixelFormat)picref->format;
    ...
    av_image_copy(c->picref->data, c->picref->linesize,
                  (const uint8_t**)picref->data, picref->linesize,
                  (PixelFormat)picref->format, picref->video->w, picref->video->h);
...
{
    BufferSourceContext *c = (BufferSourceContext*)ctx->priv;
    ...
    *c->sws_param = 0;
    ...
    if ((c->pix_fmt = av_get_pix_fmt(pix_fmt_str)) == PIX_FMT_NONE) {
        char *tail;
        c->pix_fmt = (PixelFormat)strtol(pix_fmt_str, &tail, 10);
...
{
    BufferSourceContext *c = (BufferSourceContext*)link->src->priv;
