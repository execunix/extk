...
{
    AVFilterBufferRef *ret = (AVFilterBufferRef*)av_malloc(sizeof(AVFilterBufferRef));
    if (!ret)
        return NULL;
    *ret = *ref;
    if (ref->type == AVMEDIA_TYPE_VIDEO) {
        ret->video = (AVFilterBufferRefVideoProps*)av_malloc(sizeof(AVFilterBufferRefVideoProps));
        if (!ret->video) {
            av_free(ret);
            return NULL;
        }
        *ret->video = *ref->video;
        ...
        ret->audio = (AVFilterBufferRefAudioProps*)av_malloc(sizeof(AVFilterBufferRefAudioProps));
        if (!ret->audio) {
            av_free(ret);
            return NULL;
        }
...
{
    int i;
    AVFilterPool *pool= (AVFilterPool*)ref->buf->priv;
                         ...
                         AVFilterPad *newpad)
{
    ...
    *pads  = (AVFilterPad*)av_realloc(*pads,  sizeof(AVFilterPad)   * (*count + 1));
    *links = (AVFilterLink**)av_realloc(*links, sizeof(AVFilterLink*) * (*count + 1));
                  ...
                  AVFilterContext *dst, unsigned dstpad)
{
    ...
    src->outputs[srcpad] =
    dst-> inputs[dstpad] = link = (AVFilterLink*)av_mallocz(sizeof(AVFilterLink));
...
int avfilter_config_links(AVFilterContext *filter)
{
    ...
    for (i = 0; i < filter->input_count; i ++) {
        ...
        case AVFilterLink::AVLINK_INIT:
            continue;
        case AVFilterLink::AVLINK_STARTINIT:
            av_log(filter, AV_LOG_INFO, "circular filter chain detected\n");
            return 0;
        case AVFilterLink::AVLINK_UNINIT:
            link->init_state = AVFilterLink::AVLINK_STARTINIT;
            ...
            if (link->sample_aspect_ratio.num == 0 && link->sample_aspect_ratio.den == 0)
                link->sample_aspect_ratio = link->src->input_count ?
                    link->src->inputs[0]->sample_aspect_ratio : AVRationalArg(1,1);
            ...
            link->init_state = AVFilterLink::AVLINK_INIT;
        }
    }
...
{
    AVFilterBuffer *pic = (AVFilterBuffer*)av_mallocz(sizeof(AVFilterBuffer));
    AVFilterBufferRef *picref = (AVFilterBufferRef*)av_mallocz(sizeof(AVFilterBufferRef));
    ...
    if (!(picref->video = (AVFilterBufferRefVideoProps*)av_mallocz(sizeof(AVFilterBufferRefVideoProps))))
        goto fail;
...
void avfilter_draw_slice(AVFilterLink *link, int y, int h, int slice_dir)
{
    ...
    if (link->src_buf) {
        ...
        for (i = 0; i < 4; i++) {
            int planew =
                av_image_get_linesize((PixelFormat)link->format, link->cur_buf->video->w, i);
...
void avfilter_filter_samples(AVFilterLink *link, AVFilterBufferRef *samplesref)
{
    ...
    if ((dst->min_perms & samplesref->perms) != dst->min_perms ||
        ...
        link->cur_buf = avfilter_default_get_audio_buffer(link, dst->min_perms,
                                                          (AVSampleFormat)samplesref->format,
...
{
    AVFilterContext *filter = (AVFilterContext*)p;
...
int avfilter_open(AVFilterContext **filter_ctx, AVFilter *filter, const char *inst_name)
{
    ...
    *filter_ctx = NULL;
    ...
    ret = (AVFilterContext*)av_mallocz(sizeof(AVFilterContext));
    if (!ret)
    ...
    if (ret->input_count) {
        ret->input_pads   = (AVFilterPad*)av_malloc(sizeof(AVFilterPad) * ret->input_count);
        if (!ret->input_pads)
            goto err;
        memcpy(ret->input_pads, filter->inputs, sizeof(AVFilterPad) * ret->input_count);
        ret->inputs       = (AVFilterLink**)av_mallocz(sizeof(AVFilterLink*) * ret->input_count);
        if (!ret->inputs)
            goto err;
    }
    ...
    if (ret->output_count) {
        ret->output_pads  = (AVFilterPad*)av_malloc(sizeof(AVFilterPad) * ret->output_count);
        if (!ret->output_pads)
            goto err;
        memcpy(ret->output_pads, filter->outputs, sizeof(AVFilterPad) * ret->output_count);
        ret->outputs      = (AVFilterLink**)av_mallocz(sizeof(AVFilterLink*) * ret->output_count);
        if (!ret->outputs)
            goto err;
    }
