...
static const vf_info_t* const filters[]={
    ...
    &vf_info_field,
    &vf_info_fil,
//    &vf_info_filmdint, cmmx.h vd.h ?˜opt_screen_size_x??    &vf_info_fixpts,
    &vf_info_framestep,
    &vf_info_fspp,
    &vf_info_geq,
    &vf_info_gradfun,
...
int vf_next_put_image(struct vf_instance *vf,mp_image_t *mpi, double pts){
    ...
    picref->buf = pic;
    picref->buf->please_use_av_free= (void (*)(AVFilterBuffer*))av_free;
...
{
    MPContext *m = (MPContext*)ctx->priv;
    char name[256];
    int i;
...
"Thou shalst not make spells or scripts that depend on it\n");
    ...
    if(vf->info->opts) { // vf_vo get some special argument
      ...
      if(args && !strcmp(args[0],"_oldargs_"))
        ...
        args = NULL;
#endif
    if(m->vf.info->vf_open((vf_instance*)&m->vf, (char*)args)<=0){
...
{
    MPContext *m = (MPContext*)inlink->dst->priv;
    int i;
...
{
    MPContext *m = (MPContext*)outlink->src->priv;
...
static void end_frame(AVFilterLink *inlink)
{
    ...
    int i;
    double pts= (double)MP_NOPTS_VALUE;
...
AVFilter avfilter_vf_mp = {
    ...
    inputs,
    outputs,
    NULL_IF_CONFIG_SMALL("libmpcodecs wrapper."),
};
