...
{"format_name",  "set format name",         OFFSET(format_name),  FF_OPT_TYPE_STRING, {/*.str = */0i64},  CHAR_MIN, CHAR_MAX },
{"f",            "set format name",         OFFSET(format_name),  FF_OPT_TYPE_STRING, {/*.str = */0i64},  CHAR_MIN, CHAR_MAX },
...
{
    MovieContext *movie = (MovieContext*)ctx->priv;
...
{
    MovieContext *movie = (MovieContext*)outlink->src->priv;
        ...
        if (pkt.stream_index == movie->stream_index) {
            ...
            if (frame_decoded) {
                ...
                av_image_copy(movie->picref->data, movie->picref->linesize,
                              (const uint8_t**)movie->frame->data,  movie->frame->linesize,
                              (PixelFormat)movie->picref->format, outlink->w, outlink->h);
