...
{
    TransContext *trans = (TransContext*)ctx->priv;
    trans->dir = 0;
...
static int config_props_output(AVFilterLink *outlink)
{
    ...
    if (inlink->sample_aspect_ratio.num){
        outlink->sample_aspect_ratio = av_div_q(AVRationalArg(1,1), inlink->sample_aspect_ratio);
    } else
...
{
    TransContext *trans = (TransContext*)inlink->dst->priv;
