...
AVFilterBufferRef *avfilter_default_get_video_buffer(AVFilterLink *link, int perms, int w, int h)
{
    ...
    if(pool) for(i=0; i<POOL_SIZE; i++){
        ...
        pool = link->pool = (AVFilterPool*)av_mallocz(sizeof(AVFilterPool));

    // +2 is needed for swscaler, +16 to be SIMD-friendly
    if ((i=av_image_alloc(data, linesize, w, h, (PixelFormat)link->format, 16)) < 0)
        return NULL;

    picref = avfilter_get_video_buffer_ref_from_arrays(data, linesize,
                                                       perms, w, h, (PixelFormat)link->format);
    if (!picref) {
        av_free(data[0]);
        return NULL;
    }
...
{
    AVFilterBuffer *samples = (AVFilterBuffer*)av_mallocz(sizeof(AVFilterBuffer));
    AVFilterBufferRef *ref = NULL;
    int i, sample_size, chans_nb, bufsize, per_channel_size, step_size = 0;
    char *buf;

    if (!samples || !(ref = (AVFilterBufferRef*)av_mallocz(sizeof(AVFilterBufferRef))))
        goto fail;

    ref->buf                   = samples;
    ref->format                = sample_fmt;

    ref->audio = (AVFilterBufferRefAudioProps*)av_mallocz(sizeof(AVFilterBufferRefAudioProps));
    if (!ref->audio)
        goto fail;
     ...
     */
    samples->data[0] = (uint8_t*)buf;
    if (buf && planar) {
        for (i = 1; i < chans_nb; i++) {
            step_size += per_channel_size;
            samples->data[i] = (uint8_t*)buf + step_size;
        }
    } else {
        for (i = 1; i < chans_nb; i++)
            samples->data[i] = (uint8_t*)buf;
    }
...
void avfilter_default_filter_samples(AVFilterLink *inlink, AVFilterBufferRef *samplesref)
{
    ...
    if (outlink) {
        outlink->out_buf = avfilter_default_get_audio_buffer(inlink, AV_PERM_WRITE, (AVSampleFormat)samplesref->format,
