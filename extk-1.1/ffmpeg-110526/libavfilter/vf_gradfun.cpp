...
{
    GradFunContext *gf = (GradFunContext*)ctx->priv;
    float thresh = 1.2;
    int radius = 16;
...
{
    GradFunContext *gf = (GradFunContext*)inlink->dst->priv;
    int hsub = av_pix_fmt_descriptors[inlink->format].log2_chroma_w;
    int vsub = av_pix_fmt_descriptors[inlink->format].log2_chroma_h;

    gf->buf = (uint16_t*)av_mallocz((FFALIGN(inlink->w, 16) * (gf->radius + 1) / 2 + 32) * sizeof(uint16_t));
    if (!gf->buf)
