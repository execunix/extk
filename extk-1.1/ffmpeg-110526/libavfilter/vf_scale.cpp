...
{
    ScaleContext *scale = (ScaleContext*)ctx->priv;
    const char *p;
...
static int query_formats(AVFilterContext *ctx)
{
    ...
    if (ctx->inputs[0]) {
        ...
        for (pix_fmt = 0; pix_fmt < PIX_FMT_NB; pix_fmt++)
            if (   sws_isSupportedInput((PixelFormat)pix_fmt)
    ...
    if (ctx->outputs[0]) {
        formats = NULL;
        for (pix_fmt = 0; pix_fmt < PIX_FMT_NB; pix_fmt++)
            if (    sws_isSupportedOutput((PixelFormat)pix_fmt)
                && (ret = avfilter_add_format(&formats, pix_fmt)) < 0) {
                avfilter_formats_unref(&formats);
                return ret;
            }
...
static int config_props(AVFilterLink *outlink)
{
    ...
    if (scale->sws)
        sws_freeContext(scale->sws);
    scale->sws = sws_getContext(inlink ->w, inlink ->h, (PixelFormat)inlink ->format,
                                outlink->w, outlink->h, (PixelFormat)outlink->format,
                                scale->flags, NULL, NULL, NULL);
    if (scale->isws[0])
        sws_freeContext(scale->isws[0]);
    scale->isws[0] = sws_getContext(inlink ->w, inlink ->h/2, (PixelFormat)inlink ->format,
                                    outlink->w, outlink->h/2, (PixelFormat)outlink->format,
                                    scale->flags, NULL, NULL, NULL);
    if (scale->isws[1])
        sws_freeContext(scale->isws[1]);
    scale->isws[1] = sws_getContext(inlink ->w, inlink ->h/2, (PixelFormat)inlink ->format,
                                    outlink->w, outlink->h/2, (PixelFormat)outlink->format,
                                    scale->flags, NULL, NULL, NULL);
    if (!scale->sws)
        return AVERROR(EINVAL);
...
{
    ScaleContext *scale = (ScaleContext*)link->dst->priv;
