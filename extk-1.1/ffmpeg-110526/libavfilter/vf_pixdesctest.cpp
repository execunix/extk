...
{
    PixdescTestContext *priv = (PixdescTestContext*)ctx->priv;
    av_freep(&priv->line);
}

static int config_props(AVFilterLink *inlink)
{
    PixdescTestContext *priv = (PixdescTestContext*)inlink->dst->priv;

    priv->pix_desc = &av_pix_fmt_descriptors[inlink->format];

    if (!(priv->line = (uint16_t*)av_malloc(sizeof(*priv->line) * inlink->w)))
