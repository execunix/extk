...
//      case AVMEDIA_TYPE_TEXT : ffio_wfourcc(pb, "txts"); break;
      ...
      if(stream->codec_type != AVMEDIA_TYPE_DATA){
        ...
        if (pb->seekable) {
            ...
            ffio_wfourcc(pb, avi_stream2fourcc((char*)tag, i, stream->codec_type));
                                    /* dwChunkId */
        ...
        if(   stream->codec_type == AVMEDIA_TYPE_VIDEO
           ...
           && s->streams[i]->sample_aspect_ratio.den>0){
            ...
            AVRational dar = av_mul_q(s->streams[i]->sample_aspect_ratio,
                                      AVRationalArg(stream->width, stream->height));
            int num, den;
...
//        av_log(s, AV_LOG_DEBUG, "dup %"PRId64" %d\n", pkt->dts, avist->packet_count);
    ...
    if (s->pb->seekable) {
        ...
        if (idx->ents_allocated <= idx->entry) {
            idx->cluster = (AVIIentry**)av_realloc(idx->cluster, (cl+1)*sizeof(void*));
            if (!idx->cluster)
                return -1;
