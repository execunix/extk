...
{
    struct SAPState *sap = (SAPState*)s->priv_data;
    int i;

    for (i = 0; i < s->nb_streams; i++) {
        AVFormatContext *rtpctx = (AVFormatContext*)s->streams[i]->priv_data;
        if (!rtpctx)
            continue;
...
static int sap_write_header(AVFormatContext *s)
{
    ...
    contexts = (AVFormatContext**)av_mallocz(sizeof(AVFormatContext*) * s->nb_streams);
    if (!contexts) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    sap->ann_size = 8192;
    sap->ann = (uint8_t*)av_mallocz(sap->ann_size);
    if (!sap->ann) {
        ret = AVERROR(EIO);
        goto fail;
    }
    ...
    if (av_sdp_create(contexts, s->nb_streams, (char*)&sap->ann[pos],
                       sap->ann_size - pos)) {
...
static int sap_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    rtpctx = (AVFormatContext*)s->streams[pkt->stream_index]->priv_data;
