...
int ff_sdp_parse(AVFormatContext *s, const char *content)
{
    ...
    rt->p = (pollfd*)av_malloc(sizeof(struct pollfd)*2*(rt->nb_rtsp_streams+1));
...
void ff_rtsp_undo_setup(AVFormatContext *s)
{
    ...
    for (i = 0; i < rt->nb_rtsp_streams; i++) {
        ...
        if (rtsp_st->transport_priv) {
            if (s->oformat) {
                AVFormatContext *rtpctx = (AVFormatContext*)rtsp_st->transport_priv;
                ...
                ff_rdt_parse_close((RDTDemuxContext*)rtsp_st->transport_priv);
            else if (CONFIG_RTPDEC)
                rtp_parse_close((RTPDemuxContext*)rtsp_st->transport_priv);
        }
...
static int rtsp_open_transport_ctx(AVFormatContext *s, RTSPStream *rtsp_st)
{
    ...
    if (!rtsp_st->transport_priv) {
        ...
        if (rtsp_st->dynamic_handler) {
            rtp_parse_set_dynamic_protocol((RTPDemuxContext*)rtsp_st->transport_priv,
                                           rtsp_st->dynamic_protocol_context,
                                           rtsp_st->dynamic_handler);
        }
    }
...
static void rtsp_parse_transport(RTSPMessageHeader *reply, const char *p)
{
    ...
    for (;;) {
        ...
        get_word_sep(transport_protocol, sizeof(transport_protocol),
                     "/", &p);
        if (!_stricmp(transport_protocol, "rtp")) {
        ...
        } else if (!_stricmp(transport_protocol, "x-pn-tng") ||
                   !_stricmp(transport_protocol, "x-real-rdt")) {
            /* x-pn-tng/<protocol> */
            get_word_sep(lower_transport, sizeof(lower_transport), "/;,", &p);
            profile[0] = '\0';
            th->transport = RTSP_TRANSPORT_RDT;
        }
        if (!_stricmp(lower_transport, "TCP"))
                       ...
                       int return_on_interleaved_data, const char *method)
{
    ...
    for (;;) {
        ...
        *q = '\0';
        ...
        if (line_count == 0) {
            ...
            get_word(buf1, sizeof(buf1), &p);
            get_word(buf1, sizeof(buf1), &p);
            reply->status_code = (RTSPStatusCode)atoi(buf1);
    ...
    if (content_length > 0) {
        ...
        content = (unsigned char*)av_malloc(content_length + 1);
...
redirect:
    ...
    for (rt->server_type = RTSP_SERVER_RTP;;) {
        ...
        if (rt->server_type != RTSP_SERVER_REAL && reply->real_challenge[0]) {
            ...
            continue;
        } else if (!_strnicmp(reply->server, "WMServer/", 9)) {
            rt->server_type = RTSP_SERVER_WMS;
        } else if (rt->server_type == RTSP_SERVER_REAL)
...
int ff_rtsp_fetch_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (rt->cur_transport_priv) {
        if (rt->transport == RTSP_TRANSPORT_RDT) {
            ret = ff_rdt_parse_packet((RDTDemuxContext*)rt->cur_transport_priv, pkt, NULL, 0);
        } else
            ret = rtp_parse_packet((RTPDemuxContext*)rt->cur_transport_priv, pkt, NULL, 0);
        if (ret == 0) {
    ...
    if (rt->transport == RTSP_TRANSPORT_RTP) {
        ...
        for (i = 0; i < rt->nb_rtsp_streams; i++) {
            RTPDemuxContext *rtpctx = (RTPDemuxContext*)rt->rtsp_streams[i]->transport_priv;
            int64_t queue_time;
            if (!rtpctx)
                continue;
 ...
 redo:
    if (!rt->recvbuf) {
        rt->recvbuf = (uint8_t*)av_malloc(RECVBUF_SIZE);
        if (!rt->recvbuf)
            return AVERROR(ENOMEM);
    }
    ...
    case RTSP_LOWER_TRANSPORT_UDP_MULTICAST:
        ...
        if (len > 0 && rtsp_st->transport_priv && rt->transport == RTSP_TRANSPORT_RTP)
            rtp_check_and_send_back_rr((RTPDemuxContext*)rtsp_st->transport_priv, len);
        break;
    }
    if (len == AVERROR(EAGAIN) && first_queue_st &&
        rt->transport == RTSP_TRANSPORT_RTP) {
        rtsp_st = first_queue_st;
        ret = rtp_parse_packet((RTPDemuxContext*)rtsp_st->transport_priv, pkt, NULL, 0);
        goto end;
    }
    if (len < 0)
        return len;
    if (len == 0)
        return AVERROR_EOF;
    if (rt->transport == RTSP_TRANSPORT_RDT) {
        ret = ff_rdt_parse_packet((RDTDemuxContext*)rtsp_st->transport_priv, pkt, &rt->recvbuf, len);
    } else {
        ret = rtp_parse_packet((RTPDemuxContext*)rtsp_st->transport_priv, pkt, &rt->recvbuf, len);
        if (ret < 0) {
            ...
            if (rtpctx->first_rtcp_ntp_time != AV_NOPTS_VALUE) {
                ...
                if (rtsp_st->stream_index >= 0)
                    st = s->streams[rtsp_st->stream_index];
                for (i = 0; i < rt->nb_rtsp_streams; i++) {
                    RTPDemuxContext *rtpctx2 = (RTPDemuxContext*)rt->rtsp_streams[i]->transport_priv;
                    AVStream *st2 = NULL;
...
{
    const char *p = (char*)p1->buf, *p_end = (char*)p1->buf + p1->buf_size;
...
static int sdp_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    /* XXX: better loading */
    content = (char*)av_malloc(SDP_MAX_SIZE);
