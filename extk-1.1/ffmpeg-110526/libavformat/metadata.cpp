...
int av_metadata_set2(AVMetadata **pm, const char *key, const char *value, int flags)
{
    ...
    if(!m)
        m=*pm= (AVMetadata*)av_mallocz(sizeof(*m));
    ...
    if(tag){
        ...
        *tag= m->elems[--m->count];
        ...
        AVMetadataTag *tmp= (AVMetadataTag*)av_realloc(m->elems, (m->count+1) * sizeof(*m->elems));
        if(tmp){
            m->elems= tmp;
        }else
    ...
    while((mtag=av_metadata_get(*pm, "", mtag, AV_METADATA_IGNORE_SUFFIX))) {
        ...
        if (s_conv)
            for (sc=s_conv; sc->native; sc++)
                if (!_stricmp(key, sc->native)) {
                    key = sc->generic;
                    break;
                }
        if (d_conv)
            for (dc=d_conv; dc->native; dc++)
                if (!_stricmp(key, dc->generic)) {
                    key = dc->native;
                    break;
                }
