...
static int microdvd_probe(AVProbeData *p)
{
    ...
    for (i=0; i<3; i++) {
        if (sscanf((char*)ptr, "{%*d}{}%c",     &c) != 1 &&
            sscanf((char*)ptr, "{%*d}{%*d}%c",  &c) != 1 &&
            sscanf((char*)ptr, "{DEFAULT}{}%c", &c) != 1)
            return 0;
        ptr += strcspn((char*)ptr, "\n") + 1;
    }
...
static int microdvd_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    for (i=0; i<FF_ARRAY_ELEMS(microdvd->lines); i++) {
        ...
        if ((sscanf((char*)microdvd->lines[i], "{%d}{}%6lf",    &frame, &fps) == 2 ||
             sscanf((char*)microdvd->lines[i], "{%d}{%*d}%6lf", &frame, &fps) == 2)
            && frame <= 1 && fps > 3 && fps < 100)
            pts_info = av_d2q(fps, 100000);
        if (sscanf((char*)microdvd->lines[i], "{DEFAULT}{}%c", &c) == 1) {
            st->codec->extradata = (uint8_t*)av_strdup((char*)microdvd->lines[i] + 11);
            st->codec->extradata_size = strlen((char*)st->codec->extradata);
            i--;
        }
    }
...
static int microdvd_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (i=0; i<FF_ARRAY_ELEMS(microdvd->lines); i++) {
        if (microdvd->lines[i][0]) {
            strcpy(buffer, (char*)microdvd->lines[i]);
            pos = microdvd->pos[i];
            len = strlen(buffer);
