...
int ff_sauce_read(AVFormatContext *avctx, uint64_t *fsize, int *got_width, int get_height)
{
    ...
    if (avio_read(pb, (unsigned char*)buf, 7) != 7)
        return -1;
    if (memcmp(buf, "SAUCE00", 7))
        return -1;

#define GET_SAUCE_META(name,size) \
    if (avio_read(pb, (unsigned char*)buf, size) == size && buf[0]) { \
        buf[size] = 0; \
    ...
    *fsize -= 128;
    ...
    if (nb_comments > 0) {
        ...
        if (avio_read(pb, (unsigned char*)buf, 5) == 5 && !memcmp(buf, "COMNT", 5)) {
            int i;
            ...
            *fsize -= 64*nb_comments + 5;
            ...
            for (i = 0; i < nb_comments; i++) {
                if (avio_read(pb, (unsigned char*)str + 65*i, 64) != 64)
                    break;
                str[65*i + 64] = '\n';
            }
            str[65*i] = 0;
