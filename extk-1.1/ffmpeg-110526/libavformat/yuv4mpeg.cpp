...
static int yuv4_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (*first_pkt) {
        *first_pkt = 0;
        if (yuv4_generate_header(s, buf2) < 0) {
            ...
            avio_write(pb, (unsigned char*)buf2, strlen(buf2));
        }
    }
    ...
    avio_write(pb, (unsigned char*)buf1, strlen(buf1));
...
static int yuv4_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    AVStream *st;
    struct frame_attributes *s1 = (frame_attributes*)s->priv_data;
    ...
    st->sample_aspect_ratio.set(aspectn, aspectd);
...
{
    /* check file header */
    if (strncmp((char*)pd->buf, Y4M_MAGIC, sizeof(Y4M_MAGIC)-1)==0)
