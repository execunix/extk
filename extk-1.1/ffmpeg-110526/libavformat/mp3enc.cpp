...
static int id3v1_create_tag(AVFormatContext *s, uint8_t *buf)
{
    ...
    if ((tag = av_metadata_get(s->metadata, "TCON", NULL, 0))) { //genre
        for(i = 0; i <= ID3v1_GENRE_MAX; i++) {
            if (!_stricmp(tag->value, ff_id3v1_genre_str[i])) {
                buf[127] = i;
                count++;
                break;
            }
...
static int id3v2_check_write_tag(AVFormatContext *s, AVMetadataTag *t, const char table[][5],
