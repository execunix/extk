...
struct ebml_master_arg : public ebml_master {
	ebml_master_arg(int64_t p, int s) { pos = p; sizebytes = s; }
};
...
static ebml_master start_ebml_master(AVIOContext *pb, unsigned int elementid, uint64_t expectedsize)
{
    ...
    return ebml_master_arg(avio_tell(pb), bytes);
}
...
static void mkv_write_simpletag(AVIOContext *pb, AVMetadataTag *t)
{
    ...
    if ((p = (uint8_t*)strrchr((char*)p, '-')) &&
        (lang = (const uint8_t*)av_convert_lang_to((char*)p + 1, AV_LANG_ISO639_2_BIBL)))
        *p = 0;
    ...
    put_ebml_string(pb, MATROSKA_ID_TAGNAME, (char*)key);
    if (lang)
...
static int ass_get_duration(const uint8_t *p)
{
    ...
    if (sscanf((const char*)p, "%*[^,],%d:%d:%d%*c%d,%d:%d:%d%*c%d",
    ...
    while (data_size) {
        ...
        for (i=0; i<3; i++, start++)
            if (!(start = (uint8_t*)memchr(start, ',', size-(start-data))))
                return max_duration;
        size -= start - data;
        sscanf((char*)data, "Dialogue: %d,", &layer);
        ...
        avio_w8(pb, 0);
        avio_write(pb, (unsigned char*)buffer, size);
...
static int srt_get_duration(uint8_t **buf)
{
    ...
    for (i=0; i<2 && !duration; i++) {
        ...
        if (sscanf((char*)*buf, "%d:%2d:%2d%*1[,.]%3d --> %d:%2d:%2d%*1[,.]%3d",
...
static const AVCodecTag* const codec_tag[] = {ff_codec_bmp_tags, ff_codec_wav_tags, 0};
...
AVOutputFormat ff_matroska_muxer = {
    ...
    codec_tag, // {ff_codec_bmp_tags, ff_codec_wav_tags, 0},
...
static const AVCodecTag* const codec_tag_matroska[] = {ff_codec_wav_tags, 0};
...
AVOutputFormat ff_matroska_audio_muxer = {
	...
	NULL, // dummy
	NULL, // interleave_packet
    codec_tag_matroska, // {ff_codec_wav_tags, 0},
};
#endif
