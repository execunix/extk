...
int url_fprintf(AVIOContext *s, const char *fmt, ...)
{
    ...
    va_end(ap);
    avio_write(s, (unsigned char*)buf, strlen(buf));
    return ret;
}
...
int ffio_fdopen(AVIOContext **s, URLContext *h)
{
    ...
    *s = (AVIOContext*)av_mallocz(sizeof(AVIOContext));
    ...
    if (ffio_init_context(*s, buffer, buffer_size,
                      ...
                      (int64_t (*)(void*,int64_t,int))ffurl_seek) < 0) {
        av_free(buffer);
        av_freep(s);
        return AVERROR(EIO);
    }
...
int ffio_rewind_with_probe_data(AVIOContext *s, unsigned char *buf, int buf_size)
{
    ...
    if (alloc_size > buf_size)
        if (!(buf = (unsigned char*)av_realloc(buf, alloc_size)))
            return AVERROR(ENOMEM);
...
static int dyn_buf_write(void *opaque, uint8_t *buf, int buf_size)
{
    ...
    if (new_allocated_size > d->allocated_size) {
        d->buffer = (uint8_t*)av_realloc(d->buffer, new_allocated_size);
        if(d->buffer == NULL)
             return AVERROR(ENOMEM);
...
int avio_close_dyn_buf(AVIOContext *s, uint8_t **pbuffer)
{
    ...
    if (!s->max_packet_size) {
        avio_write(s, (unsigned char*)padbuf, sizeof(padbuf));
