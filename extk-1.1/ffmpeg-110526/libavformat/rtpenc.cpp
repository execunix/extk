...
static int rtp_write_header(AVFormatContext *s1)
{
    ...
    if (s1->max_delay) {
        ...
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            s->max_frames_per_packet = av_rescale_q(s1->max_delay, AVRationalArg(1, 1000000), st->codec->time_base);
        }
    }
...
static void rtcp_send_sr(AVFormatContext *s1, int64_t ntp_time)
{
    ...
    rtp_ts = av_rescale_q(ntp_time - s->first_rtcp_ntp_time, AVRationalArg(1, 1000000),
