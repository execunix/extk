...
static int64_t get_pts(const uint8_t *p)
{
    ...
    if(sscanf((const char*)p, "%*[^,],%d:%d:%d%*c%d", &hour, &min, &sec, &hsec) != 4)
...
static int read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    qsort(ass->event, ass->event_count, sizeof(*ass->event), (int (*)(const void*,const void*))event_cmp);
...
static int read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    end= (uint8_t*)strchr((char*)p, '\n');
    av_new_packet(pkt, end ? end-p+1 : strlen((char*)p));
