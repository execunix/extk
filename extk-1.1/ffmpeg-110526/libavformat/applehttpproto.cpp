...
static int parse_playlist(URLContext *h, const char *url)
{
    ...
    while (!url_feof(in)) {
        ...
        if (av_strstart(line, "#EXT-X-STREAM-INF:", &ptr)) {
            ...
            if (is_segment) {
                struct segment *seg = (segment*)av_malloc(sizeof(struct segment));
                if (!seg) {
                    ret = AVERROR(ENOMEM);
                    goto fail;
                }
                ...
                struct variant *var = (variant*)av_malloc(sizeof(struct variant));
                if (!var) {
                    ret = AVERROR(ENOMEM);
                    goto fail;
                }
...
static int applehttp_open(URLContext *h, const char *uri, int flags)
{
    ...
    s = (AppleHTTPContext*)av_mallocz(sizeof(AppleHTTPContext));
    if (!s)
