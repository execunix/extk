...
static int match_format(const char *name, const char *names)
{
    ...
    while ((p = strchr(names, ','))) {
        ...
        if (!_strnicmp(name, names, len))
            return 1;
        names = p+1;
    }
    return !_stricmp(name, names);
}
                          ...
                          unsigned int offset, unsigned int max_probe_size)
{
    ...
    for(probe_size= PROBE_BUF_MIN; probe_size<=max_probe_size && !*fmt && ret >= 0;
        ...
        /* read probe data */
        buf = (unsigned char*)av_realloc(buf, probe_size + AVPROBE_PADDING_SIZE);
...
static int try_decode_frame(AVStream *st, AVPacket *avpkt)
{
    ...
    if(!has_codec_parameters(st->codec) || !has_decode_delay_been_guessed(st)){
        ...
        case AVMEDIA_TYPE_AUDIO:
            ...
            samples = (int16_t*)av_malloc(data_size);
            if (!samples)
                goto fail;
...
static int get_std_framerate(int i){
    static const int frate_tbl[] = {24,30,60,12,15};
    if(i<60*12) return i*1001;
    else        return frate_tbl[i-60*12]*1000*12;
}
...
AVStream *av_new_stream(AVFormatContext *s, int id)
{
    ...
    if (!st)
        return NULL;
    if (!(st->info = (AVStream::Info*)av_mallocz(sizeof(*st->info)))) {
        av_free(st);
        return NULL;
    }
    ...
    st->sample_aspect_ratio.set(0,1);
                ...
                int port, const char *fmt, ...)
{
    ...
    if (fmt) {
        ...
        va_start(vl, fmt);
        _vsnprintf(str + len, size > len ? size - len : 0, fmt, vl);
        va_end(vl);
    }
    return strlen(str);
}
