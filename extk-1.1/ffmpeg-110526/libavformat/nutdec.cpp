...
static int decode_main_header(NUTContext *nut){
    ...
    if(end > avio_tell(bc) + 4){
        ...
        for(i=1; i<nut->header_count; i++){
            ...
            nut->header[i]= (uint8_t*)av_malloc(nut->header_len[i]);
...
static int decode_stream_header(NUTContext *nut){
    ...
    klass = ffio_read_varlen(bc);
    tmp = get_fourcc(bc);
    st->codec->codec_tag= tmp;
    static const AVCodecTag* const s_codec_tag_tbl[] = { ff_codec_bmp_tags, ff_nut_video_tags, 0 };
    switch(klass)
    {
        case 0:
        ...
        case 3:
            ...
            break;
        default:
            av_log(s, AV_LOG_ERROR, "unknown stream klass (%d)\n", klass);
            return -1;
    }
    if(klass<3 && st->codec->codec_id == CODEC_ID_NONE)
...
static int decode_info_header(NUTContext *nut){
    ...
    for(i=0; i<count; i++){
        ...
        if(!strcmp(type, "UTF-8")){
            ...
            if(metadata && _stricmp(name,"Uses")
               && _stricmp(name,"Depends") && _stricmp(name,"Replaces"))
...
static int read_seek(AVFormatContext *s, int stream_index, int64_t pts, int flags){
    ...
    Syncpoint dummy={0Ui64,0Ui64,/*.ts= */pts*av_q2d(st->time_base)*AV_TIME_BASE};
    Syncpoint nopts_sp= {0Ui64,/*.back_ptr= */AV_NOPTS_VALUE,/*.ts= */AV_NOPTS_VALUE};
...
static const AVCodecTag* const codec_tag[] = { ff_codec_bmp_tags, ff_nut_video_tags, ff_codec_wav_tags, ff_nut_subtitle_tags, 0 };
...
AVInputFormat ff_nut_demuxer = {
	...
	NULL, // read_play
	NULL, // read_pause
    codec_tag, // { ff_codec_bmp_tags, ff_nut_video_tags, ff_codec_wav_tags, ff_nut_subtitle_tags, 0 },
};
#endif
