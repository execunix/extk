    ...
    while(den){
        ...
        if(a2n > max || a2d > max){
            ...
            if (den*(2*x*a1.den + a0.den) > num*a1.den)
                a1.set(x*a1.num + a0.num, x*a1.den + a0.den);
            break;
        }
...
AVRational av_div_q(AVRational b, AVRational c){
    return av_mul_q(b, AVRationalArg(c.den, c.num));
}
...
AVRational av_sub_q(AVRational b, AVRational c){
    return av_add_q(b, AVRationalArg(-c.num, c.den));
}
...
#define LOG2  0.69314718055994530941723212145817656807550013436025
    ...
    int64_t den;
    if (isnan(d))
        return AVRationalArg(0,0);
    if (isinf(d))
        return AVRationalArg(d<0 ? -1:1, 0);
