                                ...
                                const enum Mode mode, const int subframe)
{
    ...
    if (mode == MODE_12k2) {
        ff_decode_10_pulses_35bits((int16_t*)pulses, fixed_sparse, gray_decode, 5, 3);
    } else if (mode == MODE_10k2) {
        decode_8_pulses_31bits((int16_t*)pulses, fixed_sparse);
    } else {
                              ...
                              AVPacket *avpkt)
{
    ...
    float *buf_out = (float*)data;                   // pointer to the output data buffer
    int i, subframe;
    float fixed_gain_factor;
...
static AVSampleFormat sample_fmts[] = {AV_SAMPLE_FMT_FLT,AV_SAMPLE_FMT_NONE};
...
AVCodec ff_amrnb_decoder = {
    ...
    sample_fmts, // {AV_SAMPLE_FMT_FLT,AV_SAMPLE_FMT_NONE},
};
