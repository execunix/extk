...
static DWORD WINAPI attribute_align_arg thread_func(LPVOID v){
...
void ff_thread_free(AVCodecContext *s){
    ThreadContext *c= (ThreadContext*)s->thread_opaque;
    int i;
...
int ff_thread_init(AVCodecContext *s){
    ...
    c= (ThreadContext*)av_mallocz(sizeof(ThreadContext)*s->thread_count);
    s->thread_opaque= c;
...
//printf("create thread %d\n", i); fflush(stdout);
        c[i].thread = (HANDLE)CreateThread(NULL, 0, thread_func, &c[i], 0, (LPDWORD)&threadid );
