                ...
                AVRational dar=
                    av_mul_q(
                        av_div_q(
                            ...
                            AVRationalArg(s1->pan_scan.width, s1->pan_scan.height)
                        ),
                        AVRationalArg(s->width, s->height));

                //we ignore the spec here and guess a bit as reality does not match the spec, see for example
                // res_change_ffmpeg_aspect.ts and sequence-display-aspect.mpg
                //issue1613, 621, 562
                if( (s1->pan_scan.width == 0 )||(s1->pan_scan.height == 0)
                   || (av_cmp_q(dar,AVRationalArg(4,3))&&av_cmp_q(dar,AVRationalArg(16,9)))){
                                ...
                                const uint8_t *buf, int buf_size)
{
    ...
    s->current_picture.pict_type= (AVPictureType)s->pict_type;
                             ...
                             const uint8_t **buf, int buf_size)
{
    ...
    if (avctx->hwaccel) {
        ...
        int start_code = -1;
        buf_end = ff_find_start_code(buf_start + 2, *buf + buf_size, (uint32_t*)&start_code);
