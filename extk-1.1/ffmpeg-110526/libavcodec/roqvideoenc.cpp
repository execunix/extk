...
static void motion_search(RoqContext *enc, int blocksize)
{
    ...
    for (i=0; i<enc->height; i+=blocksize)
        for (j=0; j<enc->width; j+=blocksize) {
            lowestdiff = eval_motion_dist(enc, j, i, motion_vect_arg(0,0),
                                          blocksize);
            bestpick.d[0] = 0;
            bestpick.d[1] = 0;
...
static int roq_encode_init(AVCodecContext *avctx)
{
    ...
    enc->this_motion4 =
        (motion_vect*)av_mallocz((enc->width*enc->height/16)*sizeof(motion_vect));

    enc->last_motion4 =
        (motion_vect*)av_malloc ((enc->width*enc->height/16)*sizeof(motion_vect));

    enc->this_motion8 =
        (motion_vect*)av_mallocz((enc->width*enc->height/64)*sizeof(motion_vect));

    enc->last_motion8 =
        (motion_vect*)av_malloc ((enc->width*enc->height/64)*sizeof(motion_vect));
...
static AVRational supported_framerates[] = {{30,1}, {0,0}};
static PixelFormat pix_fmts[] = {PIX_FMT_YUV444P, PIX_FMT_NONE};
...
AVCodec ff_roq_encoder =
{
	...
	NULL, // next
	NULL, // flush
    supported_framerates, // {{30,1}, {0,0}},
    pix_fmts, // {PIX_FMT_YUV444P, PIX_FMT_NONE},
