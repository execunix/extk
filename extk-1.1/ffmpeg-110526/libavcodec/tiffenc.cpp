                      ...
                      const void *ptr_val)
{
    ...
    if (type_sizes[type] * count <= 4) {
        tnput(&entries_ptr, count, (uint8_t*)ptr_val, type, 0);
    } else {
        bytestream_put_le32(&entries_ptr, *s->buf - s->buf_start);
        check_size(s, count * type_sizes2[type]);
        tnput(s->buf, count, (uint8_t*)ptr_val, type, 0);
    }
                        ...
                        uint8_t * dst, int n, int compr)
{
    ...
    case TIFF_RAW:
        ...
        return n;
    case TIFF_PACKBITS:
        return ff_rle_encode(dst, s->buf_size - (*s->buf - s->buf_start), (uint8_t*)src, 1, n, 2, 0xff, -1, 0);
    case TIFF_LZW:
        return ff_lzw_encode(s->lzws, (uint8_t*)src, n);
    default:
        return -1;
    }
}
...
#endif
    {
        ...
        for (i = 0; i < s->height; i++) {
            ...
            if (is_yuv){
                 ...
                 n = encode_strip(s, (int8_t*)yuv_line, ptr, bytes_per_row, s->compr);
                 i += s->subsampling[1] - 1;
            }
            else
                n = encode_strip(s, (int8_t*)p->data[0] + i * p->linesize[0],
