                                ...
                                int *data_size, AVPacket *avpkt)
{
    ...
    if (s->block_size < s->block_width * s->block_height) {
        av_free(s->tmpblock);
        if ((s->tmpblock = (uint8_t*)av_malloc(3 * s->block_width * s->block_height)) == NULL) {
    ...
    for (j = 0; j < v_blocks + (v_part ? 1 : 0); j++) {
        ...
        for (i = 0; i < h_blocks + (h_part ? 1 : 0); i++) {
            ...
            if (size == 0) {
                ...
                s->zstream.next_in   = (Bytef*)(buf + (get_bits_count(&gb) / 8));
...
static PixelFormat pix_fmts[] = {PIX_FMT_BGR24, PIX_FMT_NONE};
...
AVCodec ff_flashsv_decoder = {
    ...
    pix_fmts, // {PIX_FMT_BGR24, PIX_FMT_NONE},
