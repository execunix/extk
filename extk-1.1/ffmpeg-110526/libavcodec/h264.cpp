...
static int decode_update_thread_context(AVCodecContext *dst, const AVCodecContext *src){
    ...
    if(!inited){
        ...
        h->s.obmc_scratchpad = (uint8_t*)av_malloc(16*2*s->linesize + 8*2*s->uvlinesize);
...
int ff_h264_frame_start(H264Context *h){
    ...
    for(i = 0; i < s->avctx->thread_count; i++)
        if(h->thread_context[i] && !h->thread_context[i]->s.obmc_scratchpad)
            h->thread_context[i]->s.obmc_scratchpad = (uint8_t*)av_malloc(16*2*s->linesize + 8*2*s->uvlinesize);
        ...
        for(list=0; list<h->list_count; list++){
            ...
            if(!IS_INTERLACED(mb_type^left_type[0])){
                if(USES_LIST(left_type[0], list)){
                    ...
                    int (*ref2frm)[64] = (int(*)[64])(h->ref2frm[ h->slice_table[left_xy[0]]&(MAX_SLICES-1) ][0] + (MB_MBAFF ? 20 : 2));
