    ...
    while(buf < buf_end) {
        ...
        case AnsiContext::STATE_NORMAL:
            ...
            case 0x0C: //FF
                ...
                break;
            case 0x1B: //ESC
                s->state = AnsiContext::STATE_ESCAPE;
                break;
            default:
                draw_char(avctx, buf[0]);
            }
            break;
        case AnsiContext::STATE_ESCAPE:
            if (buf[0] == '[') {
                s->state   = AnsiContext::STATE_CODE;
                s->nb_args = 0;
                s->args[0] = 0;
            } else {
                s->state = AnsiContext::STATE_NORMAL;
                draw_char(avctx, 0x1B);
                    return -1;
                continue;
            }
            break;
        case AnsiContext::STATE_CODE:
            switch(buf[0]) {
            ...
            case ';':
                ...
                break;
            case 'M':
                s->state = AnsiContext::STATE_MUSIC_PREAMBLE;
                break;
            case '=': case '?':
                /* ignore */
                break;
            ...
            break;
        case AnsiContext::STATE_MUSIC_PREAMBLE:
