...
{"time_base", NULL, OFFSET(time_base), FF_OPT_TYPE_RATIONAL, { 0}, INT_MIN, INT_MAX},
...
{"aspect", "sample aspect ratio", OFFSET(sample_aspect_ratio), FF_OPT_TYPE_RATIONAL, { 0}, 0, 10, V|E},
...
{"request_sample_fmt", "sample format audio decoders should prefer", OFFSET(request_sample_fmt), FF_OPT_TYPE_INT, { AV_SAMPLE_FMT_NONE }, AV_SAMPLE_FMT_NONE, AV_SAMPLE_FMT_NB-1, A|D, "request_sample_fmt"},
...
#define alloc_and_copy_or_fail(obj, size, pad) \
    if (src->obj && size > 0) { \
        *(void**)&dest->obj = av_malloc(size + pad); \
        if (!dest->obj) \
            goto fail; \
