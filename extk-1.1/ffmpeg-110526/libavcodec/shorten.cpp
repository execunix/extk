...
static int allocate_buffers(ShortenContext *s)
{
    ...
    for (chan=0; chan<s->channels; chan++) {
        ...
        s->offset[chan] = (int32_t*)av_realloc(s->offset[chan], sizeof(int32_t)*FFMAX(1, s->nmean));

        s->decoded[chan] = (int32_t*)av_realloc(s->decoded[chan], sizeof(int32_t)*(s->blocksize + s->nwrap));
        for (i=0; i<s->nwrap; i++)
            s->decoded[chan][i] = 0;
        ...
        AVPacket *avpkt)
{
    ...
    if(s->max_framesize == 0){
        ...
        s->bitstream= (uint8_t*)av_fast_realloc(s->bitstream, &s->allocated_bitstream_size, s->max_framesize);
    }
