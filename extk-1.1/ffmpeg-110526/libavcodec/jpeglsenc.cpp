...
static inline void ls_encode_regular(JLSState *state, PutBitContext *pb, int Q, int err){
    ...
    map = !state->onenear && !k && (2 * state->B[Q] <= -state->N[Q]);
...
static inline void ls_encode_line(JLSState *state, PutBitContext *pb, void *last, void *cur, int last2, int w, int stride, int comp, int bits){
    ...
    while(x < w) {
        ...
        /* run mode */
        if((FFABS(D0) <= state->onenear) && (FFABS(D1) <= state->onenear) && (FFABS(D2) <= state->onenear)) {
            int RUNval, RItype, run;

            run = 0;
            RUNval = Ra;
            while(x < w && (FFABS(R(cur, x) - RUNval) <= state->onenear)){
                run++;
                W(cur, x, Ra);
                x += stride;
            }
            ...
            if(x >= w)
                return;
            Rb = R(last, x);
            RItype = (FFABS(Ra - Rb) <= state->onenear);
            pred = RItype ? Ra : Rb;
            err = R(cur, x) - pred;
            ...
            if(state->onenear){
                if(err > 0)
                    err = (state->onenear + err) / state->twonear;
                else
                    err = -(state->onenear - err) / state->twonear;
...
static int encode_picture_ls(AVCodecContext *avctx, unsigned char *buf, int buf_size, void *data){
    JpeglsContext * const s = (JpeglsContext*)avctx->priv_data;
    AVFrame *pict = (AVFrame*)data;
    AVFrame * const p= (AVFrame*)&s->picture;
    const int onenear = avctx->prediction_method;
...
static PixelFormat pix_fmts[] = {PIX_FMT_BGR24, PIX_FMT_RGB24, PIX_FMT_GRAY8, PIX_FMT_GRAY16, PIX_FMT_NONE};
...
AVCodec ff_jpegls_encoder = { //FIXME avoid MPV_* lossless JPEG should not need them
    ...
    pix_fmts, // {PIX_FMT_BGR24, PIX_FMT_RGB24, PIX_FMT_GRAY8, PIX_FMT_GRAY16, PIX_FMT_NONE},
