...
static av_cold int aac_encode_init(AVCodecContext *avctx)
{
    ...
    s->samples            = (int16_t*)av_malloc(2 * 1024 * avctx->channels * sizeof(s->samples[0]));
    s->cpe                = (ChannelElement*)av_mallocz(sizeof(ChannelElement) * aac_chan_configs[avctx->channels-1][0]);
    avctx->extradata      = (uint8_t*)av_mallocz(5 + FF_INPUT_BUFFER_PADDING_SIZE);
    ...
    if (data) {
        if (!s->psypp) {
            ...
            for (i = 0; i < chan_map[0]; i++) {
                ...
                ff_psy_preprocess(s->psypp, (int16_t*)data + start_ch,
