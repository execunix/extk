...
{
    if (che_pos[type][id]) {
        if (!ac->che[type][id] && !(ac->che[type][id] = (ChannelElement*)av_mallocz(sizeof(ChannelElement))))
...
static int decode_cce(AACContext *ac, GetBitContext *gb, ChannelElement *che)
{
    ...
    *(int*)coup->coupling_point += get_bits1(gb) || (coup->coupling_point >> 1);
    ...
    *data_size = data_size_tmp;
    ...
    if (samples) {
        if (avctx->sample_fmt == AV_SAMPLE_FMT_FLT)
            ac->fmt_conv.float_interleave((float*)data, (const float **)ac->output_data,
                                          samples, avctx->channels);
        else
            ac->fmt_conv.float_to_int16_interleave((int16_t*)data, (const float **)ac->output_data,
                                                   samples, avctx->channels);
    }
...
static int latm_decode_frame(AVCodecContext *avctx, void *out, int *out_size,
                             AVPacket *avpkt)
{
    struct LATMContext *latmctx = (LATMContext*)avctx->priv_data;
    int                 muxlength, err;
    GetBitContext       gb;
...
{
    LATMContext *latmctx = (LATMContext*)avctx->priv_data;
    int ret;
...
static AVSampleFormat sample_fmts[] = {AV_SAMPLE_FMT_FLT,AV_SAMPLE_FMT_S16,AV_SAMPLE_FMT_NONE};
...
AVCodec ff_aac_decoder = {
    ...
    sample_fmts, // {AV_SAMPLE_FMT_FLT,AV_SAMPLE_FMT_S16,AV_SAMPLE_FMT_NONE},
    aac_channel_layout,
};
