 ...
 *                 current codeword
 */
static void do_adaptive_prediction(G722Context::G722Band *band, const int cur_diff)
{
...
static void update_low_predictor(G722Context::G722Band *band, const int ilow)
{
...
static void update_high_predictor(G722Context::G722Band *band, const int dhigh,
                                  const int ihigh)
{
...
static av_cold int g722_init(AVCodecContext * avctx)
{
    ...
    if (avctx->trellis) {
        ...
        int i;
        for (i = 0; i < 2; i++) {
            c->paths[i] = (G722Context::TrellisPath*)av_mallocz(max_paths * sizeof(**c->paths));
            c->node_buf[i] = (G722Context::TrellisNode*)av_mallocz(2 * frontier * sizeof(**c->node_buf));
            c->nodep_buf[i] = (G722Context::TrellisNode**)av_mallocz(2 * frontier * sizeof(**c->nodep_buf));
        }
    }
...
static inline int encode_high(const G722Context::G722Band *state, int xhigh)
{
...
static inline int encode_low(const G722Context::G722Band* state, int xlow)
{
                               ...
                               uint8_t *dst, int buf_size, void *data)
{
    ...
    int i, j, k;
    int frontier = 1 << avctx->trellis;
    G722Context::TrellisNode **nodes[2];
    G722Context::TrellisNode **nodes_next[2];
    int pathn[2] = {0, 0}, froze = -1;
    G722Context::TrellisPath *p[2];

    for (i = 0; i < 2; i++) {
        nodes[i] = (G722Context::TrellisNode**)c->nodep_buf[i];
        nodes_next[i] = (G722Context::TrellisNode**)c->nodep_buf[i] + frontier;
        memset(c->nodep_buf[i], 0, 2 * frontier * sizeof(*c->nodep_buf));
        nodes[i][0] = (G722Context::TrellisNode*)c->node_buf[i] + frontier;
        nodes[i][0]->ssd = 0;
        nodes[i][0]->path = 0;
    ...
    for (i = 0; i < buf_size >> 1; i++) {
        int xlow, xhigh;
        G722Context::TrellisNode *next[2];
        int heap_pos[2] = {0, 0};
        ...
        for (j = 0; j < frontier && nodes[0][j]; j++) {
            ...
            G722Context::TrellisNode *cur_node = nodes[0][j];
...
#define STORE_NODE(index, UPDATE, VALUE)\
                ...
                while (pos > 0) {\
                    ...
                    if (nodes_next[index][parent]->ssd <= ssd)\
                        break;\
                    FFSWAP(G722Context::TrellisNode*, nodes_next[index][parent],\
        ...
        for (j = 0; j < frontier && nodes[1][j]; j++) {
            int ihigh;
            G722Context::TrellisNode *cur_node = nodes[1][j];
        ...
        for (j = 0; j < 2; j++) {
            FFSWAP(G722Context::TrellisNode**, nodes[j], nodes_next[j]);
...
static AVSampleFormat sample_fmts[] = {AV_SAMPLE_FMT_S16,AV_SAMPLE_FMT_NONE};
...
AVCodec ff_adpcm_g722_encoder = {
    ...
    NULL_IF_CONFIG_SMALL("G.722 ADPCM"),
	NULL, // supported_samplerates
    sample_fmts, // {AV_SAMPLE_FMT_S16,AV_SAMPLE_FMT_NONE},
};
#endif
